        ...
        for m in modifies {
            ...
            match m {
                Modify::Delete(cf, k) => {
                    ...
                    delete.set_key(k.encoded().to_owned().into_vec());
                    if cf != CF_DEFAULT {
                ...
                Modify::Put(cf, k, v) => {
                    ...
                    put.set_key(k.encoded().to_owned().into_vec());
                    put.set_value(v);
                    if cf != CF_DEFAULT {
                ...
                Modify::DeleteRange(cf, start_key, end_key) => {
                    ...
                    delete_range.set_start_key(start_key.encoded().to_owned().into_vec());
                    delete_range.set_end_key(end_key.encoded().to_owned().into_vec());
